  ...
  size_t current_alignment)
{
  ...
  if (is_plain) {
    ...
    using DataType = carla_ros_scenario_runner_types::msg::CarlaScenario;
    is_plain =
      (
      offsetof(DataType, scenario_file) +
      last_member_size
      ) == ret_val;
  }
...
static size_t _CarlaScenario__max_serialized_size(char & bounds_info)
{
  bool full_bounded;
  bool is_plain;
  size_t ret_val;

  ret_val = max_serialized_size_CarlaScenario(full_bounded, is_plain, 0);
