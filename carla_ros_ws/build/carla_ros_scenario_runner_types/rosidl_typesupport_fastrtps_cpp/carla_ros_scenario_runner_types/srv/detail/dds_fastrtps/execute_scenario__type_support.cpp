  ...
  size_t current_alignment)
{
  ...
  if (is_plain) {
    ...
    using DataType = carla_ros_scenario_runner_types::srv::ExecuteScenario_Request;
    is_plain =
      (
...
static size_t _ExecuteScenario_Request__max_serialized_size(char & bounds_info)
{
  bool full_bounded;
  bool is_plain;
  size_t ret_val;

  ret_val = max_serialized_size_ExecuteScenario_Request(full_bounded, is_plain, 0);
  ...
  size_t current_alignment)
{
  ...
  if (is_plain) {
    ...
    // We still need to check that the in-memory alignment
    // is the same as the CDR mandated alignment.
    using DataType = carla_ros_scenario_runner_types::srv::ExecuteScenario_Response;
    is_plain =
      (
...
static size_t _ExecuteScenario_Response__max_serialized_size(char & bounds_info)
{
  bool full_bounded;
  bool is_plain;
  size_t ret_val;

  ret_val = max_serialized_size_ExecuteScenario_Response(full_bounded, is_plain, 0);
