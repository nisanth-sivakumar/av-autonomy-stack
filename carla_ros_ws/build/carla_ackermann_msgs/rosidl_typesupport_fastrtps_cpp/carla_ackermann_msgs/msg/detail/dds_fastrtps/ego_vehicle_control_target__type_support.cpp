  ...
  size_t current_alignment)
{
  ...
  if (is_plain) {
    ...
    using DataType = carla_ackermann_msgs::msg::EgoVehicleControlTarget;
    is_plain =
      (
      offsetof(DataType, jerk) +
      last_member_size
      ) == ret_val;
  }
...
static size_t _EgoVehicleControlTarget__max_serialized_size(char & bounds_info)
{
  bool full_bounded;
  bool is_plain;
  size_t ret_val;

  ret_val = max_serialized_size_EgoVehicleControlTarget(full_bounded, is_plain, 0);
