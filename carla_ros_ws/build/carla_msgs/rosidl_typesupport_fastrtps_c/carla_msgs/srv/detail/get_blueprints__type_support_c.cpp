  ...
  size_t current_alignment)
{
  ...
  if (is_plain) {
    ...
    using DataType = carla_msgs__srv__GetBlueprints_Request;
    is_plain =
      (
      offsetof(DataType, filter) +
      last_member_size
      ) == ret_val;
  }
...
static size_t _GetBlueprints_Request__max_serialized_size(char & bounds_info)
{
  bool full_bounded;
  bool is_plain;
  size_t ret_val;

  ret_val = max_serialized_size_carla_msgs__srv__GetBlueprints_Request(
  ...
  void * untyped_ros_message)
{
  ...
  // Field name: blueprints
  {
    ...
    if (!rosidl_runtime_c__String__Sequence__init(&ros_message->blueprints, size)) {
      fprintf(stderr, "failed to create array for field 'blueprints'");
      return false;
    }
    auto array_ptr = ros_message->blueprints.data;
    for (size_t i = 0; i < size; ++i) {
  ...
  size_t current_alignment)
{
  ...
  if (is_plain) {
    ...
    // We still need to check that the in-memory alignment
    // is the same as the CDR mandated alignment.
    using DataType = carla_msgs__srv__GetBlueprints_Response;
    is_plain =
      (
      offsetof(DataType, blueprints) +
      last_member_size
      ) == ret_val;
  }
...
static size_t _GetBlueprints_Response__max_serialized_size(char & bounds_info)
{
  bool full_bounded;
  bool is_plain;
  size_t ret_val;

  ret_val = max_serialized_size_carla_msgs__srv__GetBlueprints_Response(
    full_bounded, is_plain, 0);
