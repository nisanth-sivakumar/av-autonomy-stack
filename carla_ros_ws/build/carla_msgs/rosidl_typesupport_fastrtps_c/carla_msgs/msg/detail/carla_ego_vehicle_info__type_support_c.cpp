  ...
  void * untyped_ros_message)
{
  ...
  // Field name: wheels
  {
    ...
    if (!carla_msgs__msg__CarlaEgoVehicleInfoWheel__Sequence__init(&ros_message->wheels, size)) {
      fprintf(stderr, "failed to create array for field 'wheels'");
      return false;
    }
    auto array_ptr = ros_message->wheels.data;
    for (size_t i = 0; i < size; ++i) {
  ...
  size_t current_alignment)
{
  ...
  if (is_plain) {
    ...
    using DataType = carla_msgs__msg__CarlaEgoVehicleInfo;
    is_plain =
      (
      offsetof(DataType, center_of_mass) +
      last_member_size
      ) == ret_val;
  }
...
static size_t _CarlaEgoVehicleInfo__max_serialized_size(char & bounds_info)
{
  bool full_bounded;
  bool is_plain;
  size_t ret_val;

  ret_val = max_serialized_size_carla_msgs__msg__CarlaEgoVehicleInfo(
