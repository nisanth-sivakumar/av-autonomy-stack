  ...
  void * untyped_ros_message)
{
  ...
  // Field name: actors
  {
    ...
    if (!carla_msgs__msg__CarlaActorInfo__Sequence__init(&ros_message->actors, size)) {
      fprintf(stderr, "failed to create array for field 'actors'");
      return false;
    }
    auto array_ptr = ros_message->actors.data;
    for (size_t i = 0; i < size; ++i) {
  ...
  size_t current_alignment)
{
  ...
  if (is_plain) {
    ...
    using DataType = carla_msgs__msg__CarlaActorList;
    is_plain =
      (
...
static size_t _CarlaActorList__max_serialized_size(char & bounds_info)
{
  ...
  ret_val = max_serialized_size_carla_msgs__msg__CarlaActorList(
