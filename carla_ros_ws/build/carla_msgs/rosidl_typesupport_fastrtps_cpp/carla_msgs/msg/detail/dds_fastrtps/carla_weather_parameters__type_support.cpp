  ...
  size_t current_alignment)
{
  ...
  if (is_plain) {
    ...
    using DataType = carla_msgs::msg::CarlaWeatherParameters;
    is_plain =
      (
      offsetof(DataType, sun_altitude_angle) +
      last_member_size
      ) == ret_val;
  }
...
static size_t _CarlaWeatherParameters__max_serialized_size(char & bounds_info)
{
  bool full_bounded;
  bool is_plain;
  size_t ret_val;

  ret_val = max_serialized_size_CarlaWeatherParameters(full_bounded, is_plain, 0);
