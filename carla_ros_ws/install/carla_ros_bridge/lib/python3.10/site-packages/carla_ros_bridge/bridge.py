...
def main(args=None):
    ...
    try:
        ...
        # Relax version check: allow compatible 0.9.x versions
        if not dist.version.startswith("0.9"):
            carla_bridge.logfatal(
                "Incompatible CARLA version. Expected 0.9.x, found: {}".format(dist.version)
            )
            sys.exit(1)
        else:
            carla_bridge.logwarn(
                "Running with CARLA version {} (expected {}). Continuing...".format(
                    dist.version, CarlaRosBridge.CARLA_VERSION
                )
            )
